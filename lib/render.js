...
function render(err, data, evs) {
  ...
  if (err) {
    if (err.code === 'dict:offline') {
      ...
      contents = h('.dictionary-err', {key: 'err'}, h('pre', {key: 'err-stack'}, err.stack));
    }
    ...
    try {
      ...
      contents = h('.dictionary-err', {key: 'err'}, h('pre', {key: 'err-stack'}, error.stack));
    }
  } else {
    contents = h('p.dictionary-placeholder', {key: 'placeholder'}, 'Search for something!');
  }
  ...
  return h('main', {key: 'main'}, [
    ...
    h('.dictionary', {key: 'dictionary'}, h('div', {key: 'contents'}, [head(data)].concat(contents)))
  ]);
...
function head(data) {
  ...
  if (pronunciation) {
    nodes.push(' ', h('span.dictionary-pronounciation.ipa', {key: 'pronounciation'}, pronunciation));
  }

  return nodes.length === 0 ? null : h('h1.dictionary-entry', {key: 'head'}, nodes);
}
...
function syllables(data) {
  ...
  function addSyllable(syllable, index) {
    nodes.push(index ? '·' : ' — ', h('em', {key: 'syllable-' + syllable}, syllable));
  }
}

function definitions(data) {
  if (data && data.length !== 0) {
    return h('ol.dictionary-definitions', {key: 'definitions'}, data.map(definition).filter(Boolean));
  }

  return h('p.dictionary-empty', {key: 'definitions-empty'}, 'No known definitions.');
}
...
function title(data, id) {
  ...
  function add(subId) {
    ...
    function fn(subvalue, index) {
      nodes.push(index ? ', ' : null, h('em', {key: subId + '-' + index}, subvalue));
    }
  }
}
